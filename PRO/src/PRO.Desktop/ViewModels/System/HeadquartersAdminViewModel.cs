...
public partial class HeadquartersAdminViewModel : ViewModelBase
{
    ...
    private async Task LoadOperationLogsAsync()
    {
        try
        {
            var logs = await _dbContext.OperationLogs
                ...
                {
                    Id = l.Id, OperatorId = l.OperatorId,
                    OperatorName = l.Operator != null ? l.Operator.Name : l.OperatorNo,
                    OperatorNo = l.OperatorNo, Module = l.Module,
                    OperationType = l.OperationType, Content = l.Content,
                    Result = l.Result, OperatedAt = l.OperatedAt
                })
                .ToListAsync();
    ...
    private async Task SaveWeChatConfigAsync()
    {
        try
        {
            ...
            {
                CorpId = WeChatCorpId, CorpSecret = WeChatCorpSecret,
                AgentId = WeChatAgentId, DefaultWebhookUrl = WeChatWebhookUrl,
    ...
    private async Task LoadWebhooksAsync()
    {
        try
        {
            ...
            {
                Id = w.Id, Name = w.Name, WebhookUrl = w.WebhookUrl,
                TriggerCondition = w.TriggerCondition, Remark = w.Remark,
                IsEnabled = w.IsEnabled, LastTestTime = w.LastTestTime, LastTestResult = w.LastTestResult
            }));
        }
    ...
    private async Task LoadEmployeesAsync()
    {
        ...
        AllEmployees = new ObservableCollection<EmployeeListItem>(employees.Select(e => new EmployeeListItem
        {
            ...
            BranchName = e.Branch?.Name ?? "", DepartmentName = e.Department?.Name ?? "",
            RoleName = e.Role?.Name ?? "", Status = e.Status, CreatedAt = e.CreatedAt
        }));
    }
    ...
    private async Task LoadBackupRecordsAsync()
    {
        ...
        {
            Id = r.Id, FileName = r.FileName, BackupType = r.BackupType,
            FileSize = r.FileSize, BackupTime = r.BackupTime, ExpireTime = r.ExpireTime, Status = r.Status
        }));
    }
